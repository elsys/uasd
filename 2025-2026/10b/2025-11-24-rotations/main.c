...
node_t* bst_rotate_left(node_t* root) {
...
node_t* bst_rotate_right(node_t* root) {
...
node_t* bst_rotate_right_left(node_t* root) {
...
node_t* bst_rotate_left_right(node_t* root) {
...
node_t* bst_push_internal(node_t* root, int value, int myvar) {
  if(root == NULL) {
    return node_create(value);
  }

  if(value < root->value) {
    root->left = bst_push_internal(root->left, value, -1);
    if(value < root->left->value) // left-left
    else // left-right
  } else if(value > root->value) {
    root->right = bst_push_internal(root->right, value, 1);
  }
  return root;
}

void bst_push(bst_t* s, int value) {
  s->root = bst_push_internal(s->root, value, 0);
  s->size++;
}
