...
/*
    ...
    векторите са сортирани по условие, то това е и най-малкият елемент като цяло.
    Увеличаваме само индекса, от чийто вектор сме взели елемент и така повтаряме
    докато не изчерпим единия вектор.

    Накрая, остава просто да добавим по ред елементите от другия.
    Със сигурност само един от двата while цикъла ще се изпълни, тъй като със сигурност сме изчерпали
    един от векторите.

    Качил съм и снимка илюстрация на този алгоритъм (merge-two-sorted-arrays.png). Ще го разгледаме отново като стигнем до MergeSort.
*/
